...
void LCD_drawRect(uint16_t left, uint16_t right, uint16_t top, uint16_t bottom, uint16_t strokeColor, uint16_t fillColor) {
	int r;

	LCD_DrawLine(left, top, left, bottom, strokeColor);
	LCD_DrawLine(left, bottom, right, bottom, strokeColor);
	LCD_DrawLine(right, bottom, right, top, strokeColor);
	LCD_DrawLine(right, top, left, top, strokeColor);

	for(r = left + 1; r < right; r++) LCD_DrawLine(r, top + 1, r, bottom - 1, fillColor);
}
